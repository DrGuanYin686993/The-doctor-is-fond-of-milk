...
###5.提取类风湿关节炎GSE55235关节炎组样本ssGSEA结果矩阵信息
ra_res=res[,11:20]
...
group_list=ifelse(hc<6,"Cluster1","Cluster2")
table(group_list)
...
save_pheatmap_pdf(cluster_hot,"GSE55235_cluster_pheatmap.pdf",width = 12,height = 11)
...
###3.构建差异分析矩阵，进行差异分析
library(tidyverse)
group_clust_list=c(rep("Cluster1",5),rep("Cluster2",5))%>%
  factor()%>%
...
library(ggplot2)
setwd("D:\\bioarticle/6.22/figure/ssGSEA immun infiltration/")
pdf("GSE55235_ssGSEA_boxplot.pdf",width = 20,height = 10)
