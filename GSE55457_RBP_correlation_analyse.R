...
load("GSE55457_RBP_pertinence_need.rdata")
exp.pl.6=as.data.frame(exp.pl.6)


setwd("D:\\bioarticle/6.22/")
library(data.table)
DERATGs=fread("DERATGs1.csv",header = F,
              data.table = F)
colnames(DERATGs)="Gene Symbol"
...
####处理得到靶基因和RBP各自矩阵
gene=as.data.frame(t(exp.pl.6[DERATGs$`Gene Symbol`,]))

library(tidyverse)
rbp_pro=protein[,5]%>%
  as.data.frame()
colnames(rbp_pro)="Gene Symbol"
rbp_pro$score=1
exp2=exp.pl.6
exp2$"Gene Symbol"=row.names(exp2)
RBP=left_join(exp2,rbp_pro,by="Gene Symbol")%>%
  na.omit("score")
row.names(RBP)=RBP$`Gene Symbol`
RBP=as.data.frame(t(RBP[,-c(24:25)]))
...
row.names(gene_RBP)=DERATGs$`Gene Symbol`
...
save_pheatmap_pdf(RBP_DERATGs_cor,"GSE55457_RBP_pheatmap.pdf",width = 20,height = 80)
